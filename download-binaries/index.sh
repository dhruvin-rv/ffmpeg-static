...
download () {
	...
	curl -f -L -# --compressed -A 'https://github.com/dhruvin-rv/ffmpeg-static binaries download script' -o "$2" "$1"
}
...
# todo: 404
download 'https://github.com/GyanD/codexffmpeg/releases/download/7.1.1/ffmpeg-7.1.1-essentials_build.7z' win32-x64.7z
echo '  extracting'
tmpdir=$(mktemp -d)
...
download $(curl 'https://evermeet.cx/ffmpeg/info/ffmpeg/7.1.1' -fsS| jq -rc '.download.zip.url') ffmpeg-darwin-x64.zip
download $(curl 'https://evermeet.cx/ffmpeg/info/ffprobe/7.1.1' -fsS| jq -rc '.download.zip.url') ffprobe-darwin-x64.zip
echo '  extracting'
